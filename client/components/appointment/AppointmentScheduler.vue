          ...
          <v-flex xs10 offset-xs1>
            <v-card>
              <v-stepper v-model="e6" vertical>
                <v-stepper-step :complete="e6 > 1" step="1" editable>Seleciona o Dia do Agendamento</v-stepper-step>
                  ...
                  <v-flex xs11 sm5>
                    ...
                    >
                      ...
                      <v-date-picker v-model="date" no-title scrollable>
                        ...
                        <v-btn text color="primary" @click="$refs.menu.save(date)" @click.native="e6 = 2">OK</v-btn>
                      </v-date-picker>
                    </v-menu>
                  </v-flex>
                </v-stepper-content>
                <v-stepper-step
                  ...
                  :complete="e6 > 2"
                  editable
                >Perido Proposto para o Agendamento</v-stepper-step>
                <v-stepper-content step="2">
                  ...
                  <v-radio-group v-model="selected" v-show="ok" v-on:change="formatDate">
                    <v-radio
                      v-for="slot in availableAMs"
                      :key="slot.id"
                      :label="`${slot.time}`"
                      @click.native="e6 = slot.step"
                      :value="`${slot.time}`"
                      :disabled="slot.state"
                    ></v-radio>
                  </v-radio-group>
                ...
                <v-stepper-step
                  ...
                  editable
                  :complete="e6 > 3"
                >Give a description of the Event</v-stepper-step>
                ...
                <v-stepper-step
                  ...
                  editable
                  :complete="e6 > 4"
                >Select the Location</v-stepper-step>
                ...
                <v-stepper-step
                  ...
                  editable
                  :complete="e6 > 5"
                >Share your contact information with us and we 'll send you a reminder</v-stepper-step>
  ...
  methods: {
    ...
    formatDate() {
      console.log(
        ...
        moment(this.date, "YYYY/MM/DD").format("DD/MM/YYYY")
      );
    ...
    setAvailableDays() {
      ...
      const remainingdays = Math.ceil(
        (endofyear.getTime() - todaydate.getTime()) / singleday
      );
    ...
    //   };
    //   axios
    //     .post("http://localhost:3000/api/appointmentCreate", newappointment)
    //     .then(response => {
            ...
            for (var i = 0; i < this.selectedSlots.length; ++i) {
              for (var x = 0; x < this.availableAMs.length; ++x) {
                if (this.date === this.selectedSlots[i].slot_date) {
                  if (
                    this.availableAMs[x].time ===
                    this.selectedSlots[i].slot_time
                  ) {
            ...
            for (var t = 0; t < this.selectedSlots.length; ++t) {
              for (var y = 0; y < this.availablePMs.length; ++y) {
                if (this.date === this.selectedSlots[t].slot_date) {
                  if (
                    this.availablePMs[y].time ===
                    this.selectedSlots[t].slot_time
                  ) {
